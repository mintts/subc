...
{
    return GetBoolArg("-testnet", false) ? 30934 : 20934;
}
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop SubYuanChain server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "SubYuanChain server stopping";
}
...
{ //  name                      actor (function)         okSafeMode threadSafe
  //  ------------------------  -----------------------  ---------- ----------
    ...
    { "getgen",                 &getgenerate,            true,      false },
    { "setgen",                 &setgenerate,            true,      false },
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: SubYuanChain-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
static string HTTPReply(int nStatus, const string& strMsg, bool keepalive)
{
    if (nStatus == HTTP_UNAUTHORIZED)
        return strprintf("HTTP/1.0 401 Authorization Required\r\n"
            "Date: %s\r\n"
            "Server: SubYuanChain-json-rpc/%s\r\n"
...
void StartRPCThreads()
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use SubYuanChaind";
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "setgen"                 && n > 0) ConvertTo<bool>(params[0]);
    if (strMethod == "setgen"                 && n > 1) ConvertTo<boost::int64_t>(params[1]);
