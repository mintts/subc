...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("SYC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("SubYuanChain:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SYC unit input in lnReqAmount, we generate the URI with BKC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SYC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
